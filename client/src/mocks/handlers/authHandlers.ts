...
export const authHandler = [
  http.post(`${MOCK_API_PREFIX}${ADMIN_API_PREFIX}/:eventId/auth`, ({params}) => {
    console.log(params.eventId);
    return new HttpResponse(null, {
      status: 200,
    });

    // http.post(`${ADMIN_API_PREFIX}/:eventId/auth`, ({cookies}) => {
  ...
  http.post<PostLoginParams, PostLoginRequestBody, any, any>(
    `${MOCK_API_PREFIX}${ADMIN_API_PREFIX}/:eventId/login`,
    async ({request}) => {
      ...
      if (password === String(VALID_PASSWORD_FOR_TEST)) {
        return new HttpResponse(null, {
          headers: {
            'Set-Cookie': 'eventToken=valid-token',
          },
        });
