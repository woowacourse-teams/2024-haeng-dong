...
function ExpenseItem({memberName, price, onBankButtonClick, clipboardText, ...divProps}: ExpenseItemProps) {
  return (
    <Flex justifyContent="spaceBetween" alignItems="center" height="2.5rem" padding="0.5rem 1rem" {...divProps}>
      <Text size="bodyBold">{memberName}</Text>
  ...
  return (
    <div css={expenseListStyle(theme)}>
      {expenseList.map((expense, index: number) => (
        <ExpenseItem key={expense.memberName + index} {...expense} />
      ))}
    </div>
  );
}
